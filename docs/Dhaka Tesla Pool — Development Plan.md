...
### Road routes on the map (after phase 8)

- **What:** The maps now draw the real road instead of a straight dashed line. The passenger sees their own trip by road with the fare estimate and on the ride card while waiting and riding. The driver sees the route still to come, from Bullet through each stop. The driver's **See destination** button is now **See route**: it draws that request's own trip in violet over the driver's route, so where the two share a road shows at a glance, with the km it adds. A one-line legend under each map names what is drawn.
- **Why:** Pooling depends on a request lying along the driver's route (FR-L3), but the map never showed a road, so the core of the demo couldn't be seen.
- **How:** The API asks OpenRouteService for road shapes (directions with geometry), one request per new layout, and caches each leg in a new `route_path_cache` table. When the map service can't answer, a leg is drawn as a straight dashed line, as distances fall back (NFR-13). Three read-only routes serve the shapes: `GET /bookings/:id/path`, `GET /driver/pool/path` and `GET /driver/requests/:bookingId/path`, plus a `path` field on `POST /fare-estimates`. A passenger only ever gets their own pickup-to-destination road, never the shared trip's route (FR-P8, NFR-9). The shapes are for drawing only: distances, matching, fares and the booking state machine are unchanged (NFR-41). Building it showed that with real roads, Rafiq's Banani → Gulshan 1 ride is a 1.15 km detour from Nusrat's route, over the 1 km limit, so with a key they don't pool. The README now gives two worked examples: the story pair with no key, and on real roads Rafiq's Gulshan 1 ride refused next to an Airport Road ride that pools. Details in `docs/lld/route-paths.md`.
